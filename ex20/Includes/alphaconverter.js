...
function extract(did){
    ...
    for(var t=400*nth;t<400*(nth+1);t++){
...
function sew(st,pn){
    ...
    for(var q=0;q<playlists[pn].contents.length;q+=2){
        ...
        var st = playlists[pn].contents[q+2];
        if((st === 1)&&(pn === 20)){
            sn = sn + 1024;
        }
        for(var k=lc;k>0;k--){
...
function certify(playlistID,contents,duration){
    var clash = []; // array which stores all clash points
    var schedule = [];
    //get show and playlist details details
    var newPLDuration = duration;
    // for (var k=0; k<100; k=k+2){
    ...
    if ( isNaN(sn) ){//playlist
        ...
        dur = bwData.duration;
         ...
         if(tr==0){
            ...
            dur = shows[sn+1024].duration;
        }
        //watchDog.eventLog("dur:" + dur);
    }
    et = endtime(st,dur);
    return et;
}
