...
class FiltrationPumpDetailViewController: UIViewController,UIGestureRecognizerDelegate{
    ...
    @IBOutlet weak var playStpBtn: UIButton!
    ...
    private func getVoltageReading(response:[AnyObject]?){
        ...
        let indicatorLocation = abs(720 - (Double(voltageValue) * pixelPerVoltage))
        
        if voltageValue >= voltageLimit {
            voltageIndicator.frame = CGRect(x: 419, y: 270, width: 92, height: 23)
        } else if voltageValue <= 0 {
            voltageIndicator.frame = CGRect(x: 419, y: 720, width: 92, height: 23)
        } else {
    ...
    private func getCurrentReading(response:[AnyObject]?){
        ...
        let indicatorLocation = abs(720 - (Double(currentValue) * pixelPerCurrent))
        
        if currentValue >= currentLimit {
            currentIndicator.frame = CGRect(x: 600, y: 270, width: 92, height: 23)
        } else if currentValue <= 0 {
            currentIndicator.frame = CGRect(x: 600, y: 720, width: 92, height: 23)
        } else {
    ...
    private func getTemperatureReading(response:[AnyObject]?){
        ...
        let indicatorLocation = 720 - (Double(temperature) * pixelPerTemperature)

        if temperature >= 100 {
            temperatureIndicator.frame = CGRect(x: 790, y: 270, width: 75, height: 23)
        } else if temperature <= 0 {
            temperatureIndicator.frame = CGRect(x: 790, y: 720, width: 75, height: 23)
        } else {
    ...
    private func getFrequencyReading(response:[AnyObject]?){
        ...
        if frequencyValue > Int(HZMax){
            ...
            frequencyIndicator.frame = CGRect(x: 212, y: 270, width: 86, height: 23)
            ...
            frequencyIndicator.frame = CGRect(x: 212, y: (720 - length), width: 86, height: 23)
    ...
    private func getManualSpeedReading(response: [AnyObject]?){
        ...
        if manualSpeedValue > Int(HZMax){
            setFrequencyHandle.frame = CGRect(x: 403, y: 267, width: 108, height: 26)
            frequencySetLabel.textColor = GREEN_COLOR
            frequencySetLabel.text = "\(HZMax)"
            readManualSpeedOncePLC = true
        } else {
            setFrequencyHandle.frame = CGRect(x: 403, y: (717 - length), width: 108, height: 26)
            frequencySetLabel.textColor = GREEN_COLOR
    ...
    @objc func changePumpSpeedFrequency(sender: UIPanGestureRecognizer){
        ...
        if touchLocation.y  > 730 {
            touchLocation.y = 730
        }
        if touchLocation.y >= 280 && touchLocation.y <= 730 {
            
            sender.view?.center.y = touchLocation.y
            
            let flowRange = 730 - Int(touchLocation.y)
    ...
    @IBAction func playStopButtonPushed(_ sender: UIButton) {
        var startStopBit = 0
        ...
        if playStpBtn.imageView?.image == #imageLiteral(resourceName: "playButton") {
    ...
    private func readPlayStopBit(startStopMode: Int) {
            ...
            if startStopMode == 1 {
                //stop
                playStpBtn.setImage(#imageLiteral(resourceName: "stopButton"), for: .normal)
                
            } else {
                //play
                playStpBtn.setImage(#imageLiteral(resourceName: "playButton"), for: .normal)
