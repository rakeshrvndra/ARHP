...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row + 1>= _shows.count ) {
        ...
        if ([[_shows objectAtIndex:indexPath.row + 1] objectForKey:@"test"]) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row + 1>= _shows.count ) {
        if ([[[_allPlaylists objectAtIndex:indexPath.row - _shows.count  + 1] objectForKey:@"number"] intValue] < 41) {
            duration = [[[_allPlaylists objectAtIndex:indexPath.row - _shows.count  + 1] objectForKey:@"duration"] intValue];
...
-(void)getCurrentScheduleAndButtonTag{
    ...
    _highestTime = 0;
    _highestIndex = 400 * _dayOfTheWeek;
...
-(void)createShowTableView{
    ...
    for (int i = 400 * _dayOfTheWeek; i < 400 * _dayOfTheWeek + 400; i+=2){
        ...
        if (_schedule == (id)[NSNull null] || [_schedule count] != 0) {
            ...
            NSLog(@"No cookie for you cause no schedule :(");
        }
...
-(void)clearDay{
    ...
    if (_schedule == (id)[NSNull null] || [_schedule count] != 0) {
    for (int i = 0; i < 400; i++){
        [_schedule replaceObjectAtIndex:(400 * _dayOfTheWeek + i) withObject:[NSNumber numberWithInt:0]];
    }
    
    [self saveSchedule];
    } else {
        NSLog(@"BOO cannot clear day, schedule is empty");
    }
}
...
-(void)addShowPlaylist{
    if (_schedule == (id)[NSNull null] || [_schedule count] != 0) {
        if([_schedule[400 * _dayOfTheWeek + 399] intValue] != 0){
            _invalidTime.alpha = 1;
            return;
        }
        ...
        _lowestTime = [_schedule[400 * _dayOfTheWeek] intValue];
    ...
    int lastDaysShowNum = [_schedule[(_dayOfTheWeek == 0 ? 2799 : (400 * (_dayOfTheWeek - 1)) + 399)] intValue];
    ...
    if( (_dayOfTheWeek == 1) && ([_schedule[0] intValue] == 0)){
        ...
        for (int i = (400 * (_dayOfTheWeek == 0 ? 6 : (_dayOfTheWeek - 1))) + 1; i < (400 * (_dayOfTheWeek == 0 ? 6 : (_dayOfTheWeek - 1))) + (_dayOfTheWeek == 0 ? 400 : 400); i+=2){
    ...
    for (int i = 400 * _dayOfTheWeek; i < 400 * _dayOfTheWeek + 398; i+=2){
        ...
        int firstNextDaysStarting = [_schedule[_dayOfTheWeek == 6 ? 0 : (400 * (_dayOfTheWeek + 1))] intValue];
        ...
        if ((_highestTime == 0) && (_selectedTime >= lastDaysEndingTime - 2400) && (myEndingTime - 2400 <= firstNextDaysStarting)){
            ...
            [_schedule removeObjectAtIndex:(400 * _dayOfTheWeek + 398)];
            [_schedule removeObjectAtIndex:(400 * _dayOfTheWeek + 398)];
            [_schedule insertObject:[NSNumber numberWithInt:_selectedTime] atIndex:400 * _dayOfTheWeek];
            
            if ([[[NSString stringWithFormat:@"%@",_selectedShow] substringToIndex:1] isEqualToString:@"p"]){
                
                [_schedule insertObject:_selectedShow atIndex:400 * _dayOfTheWeek + 1];
                
            }else{
                
                [_schedule insertObject:[NSNumber numberWithInt:[_selectedShow intValue]] atIndex:400 * _dayOfTheWeek + 1];
        ...
        if (((_selectedTime >= lastDaysEndingTime - 2400) && (_highestTime == _lowestTime) && (i == (400 * _dayOfTheWeek))  && (myEndingTime - 2400 <= firstNextDaysStarting))){
        ...
        }else if (((_selectedTime >= lastDaysEndingTime - 2400) && (_selectedTime <= _lowestTime) && (i == (400 * _dayOfTheWeek)) && (myEndingTime <= previousStaringTime))){
    ...
    [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(showInvalidTime) userInfo:nil repeats:NO];
        ...
        NSLog(@"Oh No cannot add day. schedule is empty :(");
    }
}
