...
class WallTwoViewController: UIViewController {
    ...
    func getPumpRunningStat(){
        ...
        for index in 0..<9 {
            CENTRAL_SYSTEM?.readBits(length: 1, startingRegister: Int32(WALL_2_SURGE_PUMPS_RUNNING_STATUS_START_REGISTER + (index*offset)), completion:{ (success, response) in
                if response != nil{
    ...
    func getPumpFaultStat(){
        ...
        for index in 0..<9 {
            CENTRAL_SYSTEM?.readBits(length: 1, startingRegister: Int32(WALL_2_SURGE_PUMPS_FAULT_STATUS_START_REGISTER + (index*offset)), completion:{ (success, response) in
                if response != nil{
                    let tempstatus = Int(truncating: response![0] as! NSNumber)
    ...
    private func configureScreenTextContent(){
        ...
        guard pumpModel != nil else {
            ...
            self.navigationItem.title = "WALL - 2 PUMPS"
